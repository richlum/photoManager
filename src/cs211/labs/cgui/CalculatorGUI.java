...
import ca.ubc.cs.cpsc211.photo.gui.*;
...
public class CalculatorGUI implements ActionListener, KeyListener , MouseListener, MouseMotionListener{
    ...
    private int nextPhotoButton = 0;
    private List<JButton> numpadButtons; 
    private List<Photo> thePhotoSet;
    /**
     * Constructor
     */
    ...
    public void showCalculator(Set<Photo> photoset){
    	thePhotoSet = new ArrayList<Photo>();
    	for (Photo photo : photoset){
    		thePhotoSet.add(photo);
    	}
    	showCalculator();
    }
    /**
...
//		System.out.println(e.getSource());
	...
	public class colorMarch implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			theCal = Calendar.getInstance();
			if (theCal.get(Calendar.SECOND)<20 ){	
				marchColor(); // one key movement animation cycle per event.
			}else if (theCal.get(Calendar.SECOND)<40){
				marchColor2();
			}else {
				//if  (theCal.get(Calendar.MINUTE)%4 == 2)  {
				marchColor3();
//			}else {
//				marchColor4();
			}
	...
	int photoCount = 0;
	public void buttonPictures(List<Photo> photos) throws PhotoDoesNotExistException{
		
		JButton theButton = buttons.get(nextButton);
		Photo photo = photos.get(photoCount);
		ImageIcon icon = new ImageIcon (photo.getImage());
		if (photoCount >= photos.size()-1){
			photoCount = 0;
		}else{
			photoCount++;
		}
		//if (numpadButtons.contains(theButton) ) theButton.setIcon(icon);
		if (nextPhotoButton+1 < buttons.size()){
			nextPhotoButton++;
		}else{
			nextPhotoButton=0;
		}
